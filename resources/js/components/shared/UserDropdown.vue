    ...
    <div class="text-center">
        ...
        >
            <template v-slot:activator="{ on, attrs }">
                <v-btn color="indigo" dark fab small v-bind="attrs" v-on="on">{{ user.prenom[0] }}</v-btn>
            </template>
        ...
        <v-dialog v-model="dialog" hide-overlay persistent width="350">
            <v-card class="pa-3">
                <div class="text-center">
                    ...
                    <v-progress-circular :size="50" color="primary" indeterminate class="mt-5"></v-progress-circular>
                </div>
            </v-card>
        </v-dialog>
    </div>
    ...
    methods: {
        logout() {
            ...
            axios
                .post('logout')
                .then(() => {
                    this.$store.commit('auth/SET_LOGGED', { user: null, isLoggedIn: false });
                    location.reload();
                })
                .catch((error) => {
                    this.dialog = false;
